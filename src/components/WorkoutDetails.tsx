...
export function WorkoutDetails({ workout, onOverride, onClose }: WorkoutDetailsProps) {
  ...
  if (editing !== null) {
    ...
    const slotLabel = `S${editing.setIdx + 1} E${editing.exIdx + 1}: ${slot.exerciseName ? getShortName(slot.exerciseName) : '(empty)'}`;
              ...
              return (
                <div key={slot.id} className={styles.row} onClick={() => setEditing({ setIdx: s, exIdx: e })}>
                  ...
                  <span className={styles.rowName}>
                    {slot.exerciseName ? getShortName(slot.exerciseName) : '(empty)'}
                  </span>
